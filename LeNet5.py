...
class LeNet5(nn.Module):
    def __init__(self):
        ...
        # 包含一个卷积层和池化层，分别对应LeNet5中的C1和S2， 卷积层的输入通道为1，输出通道为6，设置卷积核大小5x5，步长为1,  池化层的kernel大小为2x2
        self._conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=6, kernel_size=5, stride=1),
            nn.MaxPool2d(kernel_size=2)
        )
        self.Sigmoid = nn.Sigmoid()
        # 包含一个卷积层和池化层，分别对应LeNet5中的C3和S4， 卷积层的输入通道为6，输出通道为16，设置卷积核大小5x5，步长为1,  池化层的kernel大小为2x2
        self._conv2 = nn.Sequential(
            nn.Conv2d(in_channels=6, out_channels=16, kernel_size=5, stride=1),
            nn.MaxPool2d(kernel_size=2)
        )
        # 对应LeNet5中C5卷积层，由于它跟全连接层类似，所以这里使用了nn.Linear模块,  卷积层的输入通特征为4x4x16，输出特征为120x1
